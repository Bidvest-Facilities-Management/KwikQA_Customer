...
export class AuthservService {
  ...
  {}
    ...
    setUser(){
        ...
        // Is this a new link?
        let apikey2 = this.findGetParameter('a') ? this.findGetParameter('a') : this.apikey;

        if (apikey2 == this.apikey) {
            this.role = this.findGetParameter('r') ? this.findGetParameter('r') : this.role;
            this.token = this.findGetParameter('t') ? this.findGetParameter('t') : this.token;
    ...
    postGEN(lclobj: any, methodname: string, classname: string = "DIESEL", url='') {
    ...
    getOTP(email = '', sms = '') {
        ...
        let lclobj = { EMAIL: email, CELLNO: sms, APIKEY: this.apikey ,ROLE:this.role};
        this.currentuser = { EMAIL: email, CELLNO: sms, APIKEY: this.apikey ,TOKEN:''};
    let tstr = this.apiserv.devprod;
        let url = 'https://io.bidvestfm.co.za/BIDVESTFM_API_ZRFC3/request?sys=' + tstr;
        this.postGEN(lclobj, 'SENDRFCOTP', 'USER', url).pipe(take(1)).
        subscribe(tokenin => {
    ...
    confirmOTP(otp = '', guid = '') {
    let lclobj = { EMAIL: this.currentuser.EMAIL, CELLNO: this.currentuser.CELLNO, OTP: otp, APIKEY: this.apikey, TOKEN: this.currentuser.TOKEN }
